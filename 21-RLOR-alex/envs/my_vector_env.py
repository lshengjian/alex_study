...
class SyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        self.n_traj = self.envs[0].unwrapped.n_traj #add by alex!
        ...
        self._rewards = np.zeros((self.num_envs, self.n_traj), dtype=np.float32)
        self._terminations = np.zeros((self.num_envs,), dtype=np.bool_)#same every traj
        self._truncations = np.zeros((self.num_envs, ), dtype=np.bool_)#same every traj
    ...
    def step(
        ...
        for i, action in enumerate(actions):
            ...
            self._terminations[i]=t1.any()
            self._truncations[i]=t2.any()
